              ...
              {ECMO_CONFIGURACIONES_ITEMS.map((item) => (
                <figure key={item.id} className="flex min-w-0 flex-col items-center">
                  <button
                    ...
                    className="group flex max-w-full cursor-zoom-in justify-center overflow-hidden rounded-2xl border border-red-100 bg-red-50/30 p-3 shadow-inner transition hover:ring-2 hover:ring-red-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
                    aria-label={`Ampliar: ${item.caption}`}
                  >
                    <img
                      src={item.src}
                      alt=""
                      className="max-h-[160px] w-auto max-w-full object-contain sm:max-h-[180px]"
                      loading="lazy"
                    />
                  </button>
                  <figcaption className="mt-3 w-full max-w-md text-center">
                    <p className="text-[11px] font-black uppercase tracking-wide text-red-900">
                      {item.caption}
                    </p>
                    <p className="mt-2 text-left text-[12px] leading-relaxed text-slate-700 sm:text-[13px]">
                      {item.description}
                    </p>
